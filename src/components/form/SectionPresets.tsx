...
export function SectionPresets({ onAddPreset, lang = 'zh' }: SectionPresetsProps) {
  ...
  const presets = [
    ...
    { type: 'summary' as const, label: isEn ? 'Summary' : '个人优势' },
  ];
          ...
          {presets.map(item => {
            ...
            return (
              <button
                ...
                className={`flex flex-col items-center justify-center gap-1.5 p-3 bg-gradient-to-b from-white to-slate-50/50 dark:from-slate-850 dark:to-slate-900 border border-slate-200/80 dark:border-slate-800 ${theme.hoverBorder} rounded-xl text-center transition-all cursor-pointer shadow-[0_1.5px_3px_rgba(15,23,42,0.01),inset_0_1.5px_2px_rgba(255,255,255,0.95)] dark:shadow-none active:translate-y-px group ${item.type === 'summary' ? 'col-span-2 md:col-span-1' : ''}`}
              >
