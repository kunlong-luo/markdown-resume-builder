...
const TRANSLATIONS = {
  ...
  en: {
    ...
    eduSuppLabel: 'Additional Notes (Optional)',
  ...
  return (
    ...
    >
      <div className="flex items-center justify-end gap-1 sm:absolute sm:right-3 sm:top-3 sm:opacity-40 sm:group-hover/item:opacity-100 transition-opacity">
        ...
        <Tooltip content={dict.moveUp} side="top" disabled={index === 0}>
        ...
        <Tooltip content={dict.moveDown} side="top" disabled={index === totalItems - 1}>
        ...
        <Tooltip content={dict.deleteItem} side="top">
      ...
      <div className="flex flex-col md:flex-row gap-2.5 sm:gap-3 sm:pr-24">
        <div className="flex-1 min-w-0">
          <div className="flex items-center justify-between mb-1">
            ...
            {(item.org || '').length > 20 && (
              <Tooltip content={lang === 'en' ? 'Long title may wrap on PDF export' : '标题较长可能会在简历单行中折行'} side="top">
                <span className="text-[10px] text-amber-500 font-medium cursor-help">
                  {lang === 'en' ? `${item.org.length} chars` : `${item.org.length}字`}
                </span>
              </Tooltip>
            )}
          </div>
        ...
        <div className="flex-1 min-w-0">
          <div className="flex items-center justify-between mb-1">
            ...
            {(item.role || '').length > 18 && (
              <Tooltip content={lang === 'en' ? 'Long role name may wrap on PDF export' : '职位名称较长可能会折行'} side="top">
                <span className="text-[10px] text-amber-500 font-medium cursor-help">
                  {lang === 'en' ? `${item.role.length} chars` : `${item.role.length}字`}
                </span>
              </Tooltip>
            )}
          </div>
        ...
        <div className="w-full md:w-[220px] shrink-0">
          <div className="mb-1">
            <label className="block text-[10px] font-extrabold text-slate-400 dark:text-slate-400 uppercase tracking-widest">{dict.periodLabel}</label>
          </div>
          <MonthRangePicker
            value={item.time || ''}
      ...
      <div>
        ...
        <div className="flex flex-col mt-1">
          ...
          <SmartMarkdownTextarea 
            ...
            placeholder={category === 'edu' ? dict.eduSuppPlaceholder : cat.contentPlaceholder} 
          />
        </div>
      </div>
    </div>
