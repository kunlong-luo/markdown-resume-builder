...
export function Editor() {
  ...
  return (
    <div className="flex flex-col h-full bg-[#fdfdfd] border-r border-gray-200 shadow-[inset_-4px_0_12px_rgb(0,0,0,0.02)] min-w-0 overflow-hidden">
      {/* Editor Header */}
      <div className="flex flex-col sm:flex-row sm:items-center justify-between px-6 py-2 bg-white/90 backdrop-blur-md border-b border-slate-200/80 gap-3 relative">
        ...
        <div className="flex bg-slate-100 p-0.5 rounded-xl border border-slate-200 shadow-inner">
          <button
            ...
            className={`flex items-center gap-1.5 px-3.5 py-1 text-xs font-bold rounded-lg transition-all duration-150 cursor-pointer ${
              activeMode === 'form'
        ...
        <div className="flex items-center gap-1.5 self-end sm:self-auto">
          <button 
            onClick={onUndo}
            disabled={!canUndo}
            className={`p-1.5 rounded-lg transition-colors ${canUndo ? 'text-slate-600 hover:text-indigo-600 hover:bg-indigo-50 cursor-pointer' : 'text-slate-300 cursor-not-allowed'}`}
          ...
          <button 
            onClick={onRedo}
            disabled={!canRedo}
            className={`p-1.5 rounded-lg transition-colors ${canRedo ? 'text-slate-600 hover:text-indigo-600 hover:bg-indigo-50 cursor-pointer' : 'text-slate-300 cursor-not-allowed'}`}
          ...
          {activeMode === 'form' && formExpandedState.hasSections && (
            <>
              <button 
                ...
                className={`flex items-center gap-1.5 px-2.5 py-1 text-xs font-bold rounded-lg border transition-all active:scale-95 cursor-pointer shadow-2xs ${
          ...
          >
            ...
            <span>{copied ? (settings.lang === 'en' ? 'Copied!' : '已复制！') : (settings.lang === 'en' ? 'Copy All' : '复制全文')}</span>
          </button>
          <div className="w-px h-4 bg-slate-200 mx-1"></div>
          <button 
            onClick={onReset}
            className="flex items-center gap-1.5 px-2.5 py-1 text-xs font-semibold text-slate-500 hover:text-rose-600 hover:bg-rose-50 rounded-lg transition-colors cursor-pointer"
            title={settings.lang === 'en' ? 'Reset to Default Template' : '重置为默认模板'}
          >
            <RotateCcw className="w-3.5 h-3.5" />
            <span>{settings.lang === 'en' ? 'Reset' : '重置'}</span>
          </button>
        </div>
      </div>
