    ...
    <div className="flex flex-col h-full bg-[#fdfdfd] border-r border-gray-200 shadow-[inset_-4px_0_12px_rgb(0,0,0,0.02)]">
        ...
        <>
          ...
          <div className="flex items-center flex-wrap gap-1 px-4 py-1.5 bg-gray-50/70 border-b border-gray-100">
            ...
            <button
              ...
              title="一键在中文与英文、数字之间添加标准空格，提升排版细节专业度"
            >
              <Type className="w-3 h-3 shrink-0" />
              <span>{spaced ? '已优化！' : '中英混排优化'}</span>
            </button>
          </div>
      ...
      <div className="flex items-center justify-between px-5 py-2 bg-gray-50 border-t border-gray-100 text-[11px] text-gray-500 font-medium z-10">
        <div className="flex items-center gap-4">
          <span>总字数: <strong className="text-gray-700">{wordCount}</strong></span>
          <span>字符数: <strong className="text-gray-700">{charCount}</strong></span>
          <span>行数: <strong className="text-gray-700">{lineCount}</strong></span>
        </div>
        <div className="flex items-center gap-3 text-blue-600">
          <div className="hidden md:flex items-center gap-1 text-gray-400 cursor-help" title="在需要强制分页的地方点击剪刀按钮插入 <!-- pagebreak -->">
            <Info className="w-3 h-3" />
            <span>支持 &lt;!-- pagebreak --&gt; 强制分页</span>
          </div>
          <div className="w-px h-3 bg-gray-200 hidden md:block"></div>
          <div className="flex items-center gap-1.5">
            <span>预估 A4 页数: <strong className="font-bold">{estPages}</strong></span>
          </div>
        </div>
      </div>
    </div>
