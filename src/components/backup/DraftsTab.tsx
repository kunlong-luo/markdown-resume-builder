...
export function DraftsTab({
  ...
  return (
    <div className="space-y-4 h-full flex flex-col">
      ...
      {/* Drafts List */}
      <div className="flex-1 min-h-[240px] overflow-y-auto border border-slate-100/80 dark:border-slate-800 rounded-xl bg-slate-50/30 dark:bg-slate-900/40 p-2 space-y-2 scrollbar-thin">
        {drafts.length === 0 ? (
          <div className="h-full flex flex-col items-center justify-center py-16 text-slate-400 dark:text-slate-500 space-y-3">
            <div className="p-3 bg-white dark:bg-slate-800 border border-slate-100 dark:border-slate-700 rounded-full shadow-sm text-slate-300 dark:text-slate-500">
              <History className="w-8 h-8 stroke-[1.5]" />
            </div>
            <div className="space-y-1 text-center">
              <p className="text-xs font-bold text-slate-700 dark:text-slate-300">{isEn ? 'No local drafts found yet' : '暂无本地草稿'}</p>
              <p className="text-[11px] text-slate-400 dark:text-slate-500 max-w-[320px] mx-auto leading-relaxed">
                {isEn 
          ...
          <div className="space-y-2.5">
            {drafts.map((draft) => (
              <div
                ...
                className="group p-4 bg-white dark:bg-slate-800 border border-slate-100 dark:border-slate-700/80 hover:border-indigo-100 dark:hover:border-indigo-800 hover:bg-indigo-50/5 dark:hover:bg-indigo-950/20 rounded-xl transition-all duration-200 cursor-pointer flex items-center justify-between shadow-2xs hover:shadow-md"
              >
                <div className="space-y-1.5 pr-4 flex-1 min-w-0">
                  {editingDraftId === draft.id ? (
                    <div className="flex items-center gap-2" onClick={e => e.stopPropagation()}>
                      <input
                        ...
                        className="bg-white dark:bg-slate-700 border border-slate-300 dark:border-slate-600 rounded-lg px-2.5 py-1 text-xs font-bold text-slate-800 dark:text-slate-100 focus:outline-none focus:ring-1 focus:ring-indigo-500"
                        autoFocus
                      />
                      <button
                        onClick={(e) => handleSaveRename(draft.id, e)}
                        className="p-1 hover:bg-emerald-100 dark:hover:bg-emerald-950/60 rounded text-emerald-600 dark:text-emerald-400"
                      >
                    ...
                    <div className="flex items-center gap-2 min-w-0">
                      <span className="font-bold text-slate-800 dark:text-slate-100 text-xs truncate group-hover:text-indigo-950 dark:group-hover:text-indigo-300 transition-colors">
                        {draft.title}
                      </span>
                      {draft.isAutoSave && (
                        <span className="px-1.5 py-0.5 bg-amber-50 dark:bg-amber-950/60 border border-amber-100 dark:border-amber-800/60 text-amber-700 dark:text-amber-300 text-[9px] rounded font-bold shrink-0">
                          {isEn ? 'Autosaved' : '自动保存'}
                        </span>
                      )}
                    </div>
                  ...
                  <div className="flex flex-wrap items-center gap-x-4 gap-y-1 text-[10px] text-slate-400 dark:text-slate-400">
                    <span className="flex items-center gap-1">
                      <Clock className="w-3 h-3 text-slate-300 dark:text-slate-500 shrink-0" />
                      {draft.timestamp}
                    </span>
                    <span className="flex items-center gap-1">
                      <FileText className="w-3 h-3 text-slate-300 dark:text-slate-500 shrink-0" />
                      {draft.markdown.length} {isEn ? 'Chars' : '字符'}
                    </span>
                    <span className="flex items-center gap-1">
                      <Settings className="w-3 h-3 text-slate-300 dark:text-slate-500 shrink-0" />
